...
class NullDistribution(TorchProbabilityDistribution):
    ...
    def estimator(self, pseudo_count: Optional[float] = None, device: Optional[str] = None) -> 'NullEstimator':
...
class NullAccumulator(TorchStatisticAccumulator):
    ...
    def seq_update(self, x: 'NullTorchEncodedSequence', weights: tn.Tensor, estimate: Optional['NullDistribution']) -> None:
        pass

    def seq_initialize(self, x: 'NullTorchEncodedSequence', weights: tn.Tensor, tng: Optional['tn.Generator']) -> None:
        pass
...
class NullEstimator(TorchParameterEstimator):
    ...
    def estimate(self, nobs: Optional[float], suff_stat: Optional[Any] = None, device: Optional[tn.device] = None) -> 'NullDistribution':
...
class NullDataEncoder(TorchSequenceEncoder):
    ...
    def seq_encode(self, x: Any, device: Optional[tn.device] = None) -> 'NullTorchEncodedSequence':
