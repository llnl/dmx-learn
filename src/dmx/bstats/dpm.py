...
class DirichletProcessMixtureDistribution(ProbabilityDistribution):
    ...
    def __init__(
        self, components, w, a, g, component_priors, name=None, prior=default_prior
    ):
    ...
    def __str__(self):
        return (
            "DirichletProcessMixtureDistribution([%s], [%s], %s, name=%s, prior=%s)"
            % (
    ...
    def seq_log_density(self, x):
        ...
        for i in range(self.max_components):
            temp2 += (
                -self.components[i].get_prior().cross_entropy(self.component_priors[i])
            )
...
class DirichletProcessMixtureSampler(object):
    ...
    def sample(self, size=None):
        ...
        compState = self.rng.choice(
            range(0, len(self.dist.w)), size=size, replace=True, p=self.dist.w
        )
...
class DirichletProcessMixtureAccumulator(StatisticAccumulator):
    ...
    def seq_update(self, x, weights, estimate):
        ...
        exp_ll = np.asarray(
            [u.seq_expected_log_density(x) for u in estimate.components]
        ).T
        exp_ll += estimate.log_w
        ...
        for i in range(self.num_components):
            self.accumulators[i].seq_update(
                x, phi[i, :] * weights, estimate.components[i]
            )
...
class DirichletProcessMixtureEstimator(object):
    ...
    def get_prior(self):
        vprior = SequenceDistribution(
            BetaDistribution(1, (self.prior.k - 1) * self.prior.theta), null_dist
        )
    ...
    def model_log_density(self, model):
        ...
        for i in range(model.max_components):
            temp2 += (
                -model.components[i]
                .get_prior()
                .cross_entropy(model.component_priors[i])
            )
