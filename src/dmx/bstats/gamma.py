...
class GammaDistribution(ProbabilityDistribution):
    ...
    def __str__(self):
        return "GammaDistribution(%f, %f, name=%s, prior=%s)" % (
            self.k,
            self.theta,
            str(self.name),
            str(self.prior),
    ...
    def cross_entropy(self, dist: ProbabilityDistribution):
        if isinstance(dist, GammaDistribution):
            ...
            return -scipy.integrate.quad(
                lambda x: dist.log_density(x) * self.density(x), 0, np.inf
            )[0]
...
class GammaEstimator(ParameterEstimator):
    ...
    def __init__(
        self, pseudo_count=(0.0, 0.0), suff_stat=(1.0, 0.0), threshold=1.0e-8, keys=None
    ):
