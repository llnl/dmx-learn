...
class BernoulliSetDistribution(ProbabilityDistribution):
    ...
    def __init__(self, pmap, name=None, prior=None):
        ...
        self.pmap = pmap
        self.log_pmap = {
            k: np.log1p(v) if v < 0 else np.log(v) for k, v in pmap.items()
        }
        self.log_nmap = {
            k: np.log(-v) if v < 0 else np.log1p(-v) for k, v in pmap.items()
        }
        self.nmap_sum = sum([u for u in self.log_nmap.values() if u != -np.inf])

    def __str__(self):
        return "BernoulliSetDistribution(%s, name=%s, prior=%s)" % (
            str(self.pmap),
            str(self.name),
            str(self.prior),
        )
    ...
    def seq_log_density(self, x):
        ...
        log_prob_loc = np.asarray(
            [self.log_pmap[u] - self.log_nmap[u] for u in val_map_inv]
        )
...
class BernoulliSetEstimator(ParameterEstimator):
    ...
    def estimate(self, suff_stat):
        ...
        if isinstance(self.prior, BetaDistribution):
            pmap = bernoulli_beta_posterior_mode(
                suff_stat[0], suff_stat[1], self.prior.get_parameters()
            )
            ...
            pmap = bernoulli_betamix_posterior_mode(
                suff_stat[0], suff_stat[1], self.prior.w, beta_params
            )
...
def bernoulli_betamix_posterior_mode(obs_cnt, tot_cnt, w, beta_params):
    ...
    for k, v in obs_cnt.items():
        ...
        ll = (
            log_w
            + gammaln(beta_params[:, 0] + v)
            + gammaln(beta_params[:, 1] + (tot_cnt - v))
            + lc
        )
        bidx = ll.argmax()
