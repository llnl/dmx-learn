...
class DiagonalGaussianDistribution(ProbabilityDistribution):
    ...
    def __init__(
        ...
        covariance: Union[Sequence[float], np.ndarray],
...
class DiagonalGaussianSampler(object):
    ...
    def sample(self, size=None):
        if size is None:
            rv = (
                self.rng.standard_normal(size=self.dist.dim) * np.sqrt(self.dist.covar)
                + self.dist.mu
            )
            return rv.tolist()
        else:
            rv = (
                self.rng.standard_normal(size=(size, self.dist.dim))
                * np.sqrt(self.dist.covar)
                + self.dist.mu
            )
...
class DiagonalGaussianEstimator(ParameterEstimator):
    ...
    def accumulator_factory(self):
        dim = self.dim
        obj = type(
            "", (object,), {"make": lambda o: DiagonalGaussianAccumulator(dim=dim)}
        )()
        return obj
    ...
    def estimate(self, suff_stat):
        ...
        if self.has_conj_prior:
            ...
            return DiagonalGaussianDistribution(
                new_mu, new_sigma2, name=self.name, prior=new_prior
            )
