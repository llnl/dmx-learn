...
class SequenceDistribution(
    ProbabilityDistribution[List[X], Tuple[P1, P2], Tuple[V1, V2]]
):
    ...
    def __str__(self):
        return "SequenceDistribution(%s, len_dist=%s, name=%s)" % (
            str(self.dist),
            str(self.len_dist),
            str(self.name),
        )
    ...
    def estimator(self):
        return SequenceEstimator(
            self.dist.estimator(), self.len_dist.estimator(), name=self.name
        )
...
class SequenceEstimator(ParameterEstimator):
    ...
    def get_prior(self):
        return CompositeDistribution(
            (self.estimator.get_prior(), self.len_estimator.get_prior())
        )
    ...
    def accumulator_factory(self):
        ...
        if self.len_estimator is None:
            obj = type(
                ...
                (object,),
                {
                    "make": lambda o: SequenceEstimatorAccumulator(
            ...
            obj = type(
                ...
                (object,),
                {
                    "make": lambda o: SequenceEstimatorAccumulator(
                        self.estimator.accumulator_factory().make(),
                        self.len_normalized,
                        self.len_estimator.accumulator_factory().make(),
                        self.keys,
                    )
                },
            )()
