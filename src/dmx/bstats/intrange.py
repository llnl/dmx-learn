...
class IntegerCategoricalDistribution(ProbabilityDistribution):
    ...
    def moment(self, p, o=0):
        return np.dot(
            np.power(np.arange(self.min_index, self.max_index + 1) - o, p),
            self.prob_vec,
        )
    ...
    def sample(self, size=None):
        ...
        if size is None:
            return self.rng.choice(
                range(self.dist.min_index, self.dist.max_index + 1),
...
class IntegerCategoricalEstimator(object):
    ...
    def accumulator_factory(self):
        ...
        obj = type(
            ...
            (object,),
            {
                "make": lambda o: IntegerCategoricalAccumulator(
                    minVal, maxVal, self.keys
                )
            },
        )()
        return obj
