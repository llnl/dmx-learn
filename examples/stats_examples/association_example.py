...
if __name__ == "__main__":
    ...
    dist = IntegerHiddenAssociationDistribution(
        state_prob_mat, cond_weights, prev_dist=init_dist, len_dist=len_dist
    )
    ...
    est = IntegerHiddenAssociationEstimator(
        4, 2, prev_estimator=prev_est, len_estimator=len_est, use_numba=False
    )

    model = optimize(data, est, max_its=1, rng=np.random.RandomState(1))
    t0 = time.time()
    model = optimize(
        data, est, max_its=500, print_iter=50, prev_estimate=model, init_p=1.0
    )
    print(time.time() - t0)
    print(str(model))
