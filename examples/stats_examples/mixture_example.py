...
if __name__ == "__main__":
    ...
    d20 = MixtureDistribution(
        [GaussianDistribution(-3.0, 1.0), GaussianDistribution(0.0, 1.0)], [0.25, 0.75]
    )
    ...
    e0 = MixtureEstimator(
        [GaussianEstimator()] * 2, keys=(None, "comps0"), pseudo_count=1.0
    )
    ...
    iest = MixtureEstimator(
        [CompositeEstimator((e0, e1, e2, e3, e4))] * 2,
        pseudo_count=1.0,
        fixed_weights=[0.2, 0.8],
    )
    ...
    est = MixtureEstimator(
        [CompositeEstimator((e0, e1, e2, e3, e4))] * 2, fixed_weights=[0.2, 0.8]
    )
