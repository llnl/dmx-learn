...
class GaussianDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        self.est_factory = [
            ...
            (
                GaussianEstimator(name="name", keys="key"),
                GaussianAccumulatorFactory(name="name", keys="key"),
            ),
            (GaussianEstimator(), GaussianAccumulatorFactory()),
        ]
        self.factory_acc = [
            (
                GaussianAccumulatorFactory(name="b", keys="key"),
                GaussianAccumulator(name="b", keys="key"),
            ),
