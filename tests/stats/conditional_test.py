"""Tests the conditional distribution."""
...
class ConditionalDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        self._default_acc = [GaussianAccumulator(), CategoricalAccumulator()]
        self._given_acc = [CategoricalAccumulator(), BinomialAccumulator()]
        ...
        self.eval_dists = [
            ...
            ConditionalDistribution(
                dmap=self._dmaps[0],
                given_dist=self._given_dists[2],
                default_dist=self._default_dists[0],
            ),
            ConditionalDistribution(
                dmap=self._dmaps[1],
                given_dist=self._given_dists[3],
                default_dist=self._default_dists[1],
            ),
        ]
        self._ests = [
            ConditionalDistributionEstimator(
                estimator_map=self._emaps[0],
                given_estimator=self._given_ests[0],
                name="c",
                keys="key",
            ),
            ConditionalDistributionEstimator(
                estimator_map=self._emaps[1],
            ...
            ConditionalDistributionEstimator(
                estimator_map=self._emaps[0],
                given_estimator=self._given_ests[0],
                default_estimator=self._default_ests[0],
            ),
        ]
        self._factories = [
            ConditionalDistributionAccumulatorFactory(
                factory_map=self._factory_map[0],
                given_factory=self._given_factory[0],
                name="c",
                keys="key",
            ),
            ConditionalDistributionAccumulatorFactory(
                factory_map=self._factory_map[1],
                given_factory=self._given_factory[1],
                name="c",
                keys="key",
            ),
            ConditionalDistributionAccumulatorFactory(
                factory_map=self._factory_map[0],
                given_factory=self._given_factory[0],
                default_factory=self._default_factory[0],
            ),
        ]
        self._accumulators = [
            ConditionalDistributionAccumulator(
                accumulator_map=self._acc_map[0],
                given_accumulator=self._given_acc[0],
                name="c",
                keys="key",
            ),
            ConditionalDistributionAccumulator(
                accumulator_map=self._acc_map[1],
                given_accumulator=self._given_acc[1],
                name="c",
                keys="key",
            ),
            ConditionalDistributionAccumulator(
                accumulator_map=self._acc_map[0],
                given_accumulator=self._given_acc[0],
                default_accumulator=self._default_acc[0],
            ),
        ]
        self._encoders = [
            ConditionalDistributionDataEncoder(
                encoder_map=self._encoder_map[0], given_encoder=self._given_encoders[0]
            ),
            ConditionalDistributionDataEncoder(
                encoder_map=self._encoder_map[1], given_encoder=self._given_encoders[1]
            ),
            ConditionalDistributionDataEncoder(
                encoder_map=self._encoder_map[0],
                default_encoder=self._default_encoders[0],
                given_encoder=self._given_encoders[0],
            ),
            ConditionalDistributionDataEncoder(
                encoder_map=self._encoder_map[1],
                default_encoder=self._default_encoders[1],
                given_encoder=self._given_encoders[1],
            ),
        ]
        ...
        self.acc_encoder = [
            (a, e) for a, e in zip(self._accumulators, self._encoders[:3])
        ]
