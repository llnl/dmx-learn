...
class MixtureDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        self._estimators = [
            ...
            CategoricalEstimator(name="c"),
            CategoricalEstimator(keys="c_keys"),
        ]
        self._factories = [
            CategoricalAccumulatorFactory(),
            CategoricalAccumulatorFactory(name="c"),
            CategoricalAccumulatorFactory(keys="c_keys"),
        ]
        self._accumulators = [
            CategoricalAccumulator(),
            CategoricalAccumulator(name="c"),
            CategoricalAccumulator(keys="c_keys"),
        ]

        self.eval_dists = [
            MixtureDistribution(
                components=self._comps[0], w=[1 / 3, 4 / 9, 2 / 9], name="mix"
            ),
            MixtureDistribution(
                components=self._comps[0][:2], w=[0.20, 0.80], name="mix"
            ),
            MixtureDistribution(components=self._comps[0][:1], w=[1] * 1, name="mix"),
            MixtureDistribution(components=self._comps[1], w=[1 / 3.0] * 3, name="mix"),
            MixtureDistribution(
                components=self._comps[1][:2], w=[1 / 2.0] * 2, name="mix"
            ),
        ...
        self.dist_est = [
            (
                self.eval_dists[0],
                MixtureEstimator(estimators=[self._estimators[0]] * 3, name="mix"),
            ),
            (
                self.eval_dists[1],
                MixtureEstimator(estimators=[self._estimators[0]] * 2, name="mix"),
            ),
            (
                self.eval_dists[2],
                MixtureEstimator(estimators=[self._estimators[0]] * 1, name="mix"),
            ),
            (
                self.eval_dists[3],
                MixtureEstimator(estimators=[self._estimators[1]] * 3, name="mix"),
            ),
            (
                self.eval_dists[4],
                MixtureEstimator(estimators=[self._estimators[1]] * 2, name="mix"),
            ),
            (
                self.eval_dists[5],
                MixtureEstimator(estimators=[self._estimators[1]] * 1, name="mix"),
            ),
        ]
        ...
        self.est_factory = [
            (
                MixtureEstimator(
                    estimators=[self._estimators[0]] * 3, name="m", keys=("w", None)
                ),
                MixtureAccumulatorFactory(
                    factories=[self._factories[0]] * 3, name="m", keys=("w", None)
                ),
            ),
            (
                MixtureEstimator(
                    estimators=[self._estimators[0]] * 1, name="m", keys=(None, "comps")
                ),
                MixtureAccumulatorFactory(
                    factories=[self._factories[0]] * 1, name="m", keys=(None, "comps")
                ),
            ),
            (
                MixtureEstimator(
                    estimators=[self._estimators[1]] * 3, name="m", keys=("w", "comps")
                ),
                MixtureAccumulatorFactory(
                    factories=[self._factories[1]] * 3, name="m", keys=("w", "comps")
                ),
            ),
            (
                MixtureEstimator(estimators=[self._estimators[1]] * 1, name="m"),
                MixtureAccumulatorFactory(
                    factories=[self._factories[1]] * 1, name="m", keys=(None, None)
                ),
            ),
            (
                MixtureEstimator(estimators=[self._estimators[2]] * 3),
                MixtureAccumulatorFactory(factories=[self._factories[2]] * 3),
            ),
            (
                MixtureEstimator(estimators=[self._estimators[2]] * 1, name="m"),
                MixtureAccumulatorFactory(factories=[self._factories[2]] * 1, name="m"),
            ),
        ]
        self.factory_acc = [
            (
                ...
                MixtureAccumulator(
                    accumulators=[self._accumulators[0]] * 3, name="m", keys=("w", None)
                ),
            ),
            (
                MixtureAccumulatorFactory(
                ...
                MixtureAccumulator(
                    accumulators=[self._accumulators[0]] * 1,
                    name="m",
                    keys=(None, "comps"),
                ),
            ),
            (
                ...
                MixtureAccumulator(
                    accumulators=[self._accumulators[1]] * 3,
                    name="m",
                    keys=("w", "comps"),
                ),
            ),
            (
                MixtureAccumulatorFactory(factories=[self._factories[1]] * 1, name="m"),
                MixtureAccumulator(
                    accumulators=[self._accumulators[1]] * 1,
                    name="m",
                    keys=(None, None),
                ),
            ),
            (
                MixtureAccumulatorFactory(
                    factories=[self._factories[1]] * 1, keys=(None, None)
                ),
                MixtureAccumulator(accumulators=[self._accumulators[1]] * 1),
            ),
        ]
        self.acc_encoder = [
            (
                MixtureAccumulator(accumulators=[self._accumulators[1]] * 3),
                self._encoder,
            )
        ]
