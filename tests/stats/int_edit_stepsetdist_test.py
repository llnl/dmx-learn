    ...
    def setUp(self) -> None:
        ...
        self.eval_dists = [
            IntegerStepBernoulliEditDistribution(
                log_edit_pmat=log_edit_pmat[0],
                init_dist=init_dists[0],
                name="name",
                keys="keys",
            ),
            IntegerStepBernoulliEditDistribution(
                log_edit_pmat=log_edit_pmat[0], init_dist=init_dists[0], keys="keys"
            ),
            IntegerStepBernoulliEditDistribution(
                log_edit_pmat=log_edit_pmat[1], init_dist=init_dists[1], name="name"
            ),
            IntegerStepBernoulliEditDistribution(
                log_edit_pmat=log_edit_pmat[1], init_dist=init_dists[1]
            ),
        ]
        self._ests = [
            IntegerStepBernoulliEditEstimator(
                num_vals=3, init_estimator=init_ests[0], name="name", keys="keys"
            ),
            IntegerStepBernoulliEditEstimator(
                num_vals=3, init_estimator=init_ests[0], keys="keys"
            ),
            IntegerStepBernoulliEditEstimator(
                num_vals=4, init_estimator=init_ests[1], name="name"
            ),
            IntegerStepBernoulliEditEstimator(num_vals=4, init_estimator=init_ests[1]),
        ]
        self._factories = [
            IntegerStepBernoulliEditAccumulatorFactory(
                num_vals=3, init_factory=init_factory[0], name="name", keys="keys"
            ),
            IntegerStepBernoulliEditAccumulatorFactory(
                num_vals=3, init_factory=init_factory[0], keys="keys"
            ),
            IntegerStepBernoulliEditAccumulatorFactory(
                num_vals=4, init_factory=init_factory[1], name="name"
            ),
            IntegerStepBernoulliEditAccumulatorFactory(
                num_vals=4, init_factory=init_factory[1]
            ),
        ]
        self._accumulators = [
            IntegerStepBernoulliEditAccumulator(
                num_vals=3, init_acc=init_accumulator[0], name="name", keys="keys"
            ),
            IntegerStepBernoulliEditAccumulator(
                num_vals=3, init_acc=init_accumulator[0], keys="keys"
            ),
            IntegerStepBernoulliEditAccumulator(
                num_vals=4, init_acc=init_accumulator[1], name="name"
            ),
            IntegerStepBernoulliEditAccumulator(
                num_vals=4, init_acc=init_accumulator[1]
            ),
        ]
        self._encoders = [
            IntegerStepBernoulliEditDataEncoder(init_encoder=init_encoder)
        ] * len(self.eval_dists)
