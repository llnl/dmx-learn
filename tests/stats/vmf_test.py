    ...
    def setUp(self) -> None:
        self.eval_dists = [
            VonMisesFisherDistribution(
                mu=np.ones(3) / 3.0, kappa=1.0, name="name", keys="keys"
            ),
            VonMisesFisherDistribution(mu=np.ones(3) / 3.0, kappa=1.0, keys="keys"),
            VonMisesFisherDistribution(mu=np.ones(4) / 4.0, kappa=0.10),
        ]
        self._ests = [
