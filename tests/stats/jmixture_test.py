...
class JointMixtureDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        self._w0 = [np.ones(4) / 4.0, np.ones(2) / 2.0]
        self._w1 = [np.ones(2) / 2.0, np.ones(4) / 4.0]
        ...
        self._ests0 = [[GaussianEstimator()] * 4, [GaussianEstimator()] * 2]
        self._ests1 = [[GaussianEstimator()] * 2, [GaussianEstimator()] * 4]
        self._accs0 = [[GaussianAccumulator()] * 4, [GaussianAccumulator()] * 2]
        self._accs1 = [[GaussianAccumulator()] * 2, [GaussianAccumulator()] * 4]
        self._facs0 = [
        ...
        self._encoders = [
            JointMixtureDataEncoder(*tuple([GaussianDataEncoder()] * 2))
        ] * len(self.eval_dists)
