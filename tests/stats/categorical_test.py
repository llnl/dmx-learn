...
class CategoricalDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        self.eval_dists = [
            CategoricalDistribution(
                pmap={"a": 0.1, "b": 0.3, "c": 0.6}, default_value=0.0, name="cat"
            ),
            CategoricalDistribution(pmap={"a": 0.1, "b": 0.3, "c": 0.6}, name="cat"),
            CategoricalDistribution(
                pmap={"a": 0.1, "b": 0.3, "c": 0.6}, default_value=0.0
            ),
        ...
        self.dist_est = [
            ...
            (
                CategoricalDistribution(
                    pmap={"a": 0.1, "b": 0.3, "c": 0.6}, name="c", keys="ckey"
                ),
                CategoricalEstimator(name="c", keys="ckey"),
            ),
        ]
        ...
        self.density_dist_encoder = [
            ...
            (
                CategoricalDistribution(
                    pmap={"a": 0.1, "b": 0.3, "c": 0.6}, default_value=0.50, name="cat"
                ),
                CategoricalDataEncoder(),
            ),
        ]
        self.est_factory = [
            (
                CategoricalEstimator(name="c", keys="key"),
                CategoricalAccumulatorFactory(name="c", keys="key"),
            ),
            (
                CategoricalEstimator(keys="key"),
                CategoricalAccumulatorFactory(keys="key"),
            ),
        ...
        self.factory_acc = [
            (
                CategoricalAccumulatorFactory(name="c", keys="key"),
                CategoricalAccumulator(name="c", keys="key"),
            ),
            (
                CategoricalAccumulatorFactory(keys="key"),
                CategoricalAccumulator(keys="key"),
            ),
    ...
    def test_key_exceptions(self):
        for x in self.type_check_keys:
            ...
            assert (
                str(e.value)
                == "CategoricalEstimator requires keys to be of type 'str'."
            )
