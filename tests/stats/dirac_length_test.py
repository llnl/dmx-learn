...
class DiracMixtureDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        self._base_dists = [
            BinomialDistribution(p=0.9, n=5),
            CategoricalDistribution({3: 0.1, 4: 0.8, 5: 0.1}),
        ]
        ...
        self.eval_dists = [
            DiracMixtureDistribution(
                dist=self._base_dists[0], p=0.20, v=0, name="a", keys=("w", "comps")
            ),
            DiracMixtureDistribution(
                dist=self._base_dists[0], p=0.20, v=100, name="a", keys=("w", None)
            ),
            DiracMixtureDistribution(
                dist=self._base_dists[0], p=0.20, v=0, name="a", keys=(None, "comps")
            ),
            DiracMixtureDistribution(dist=self._base_dists[0], p=0.20),
            DiracMixtureDistribution(
                dist=self._base_dists[1], p=0.20, v=0, name="a", keys=("w", "comps")
            ),
            DiracMixtureDistribution(
                dist=self._base_dists[1], p=0.20, v=100, name="a", keys=("w", None)
            ),
            DiracMixtureDistribution(
                dist=self._base_dists[1], p=0.20, v=0, name="a", keys=(None, "comps")
            ),
        ...
        self._ests = [
            DiracMixtureEstimator(
                estimator=self._base_ests[0], v=0, name="a", keys=("w", "comps")
            ),
            DiracMixtureEstimator(
                estimator=self._base_ests[0], v=100, name="a", keys=("w", None)
            ),
            DiracMixtureEstimator(
                estimator=self._base_ests[0], v=0, name="a", keys=(None, "comps")
            ),
            DiracMixtureEstimator(estimator=self._base_ests[0]),
            DiracMixtureEstimator(
                estimator=self._base_ests[1], v=0, name="a", keys=("w", "comps")
            ),
            DiracMixtureEstimator(
                estimator=self._base_ests[1], v=100, name="a", keys=("w", None)
            ),
            DiracMixtureEstimator(
                estimator=self._base_ests[1], v=0, name="a", keys=(None, "comps")
            ),
            DiracMixtureEstimator(estimator=self._base_ests[1]),
        ]
        self._factories = [
            DiracMixtureAccumulatorFactory(
                factory=self._base_factories[0], v=0, name="a", keys=("w", "comps")
            ),
            DiracMixtureAccumulatorFactory(
                factory=self._base_factories[0], v=100, name="a", keys=("w", None)
            ),
            DiracMixtureAccumulatorFactory(
                factory=self._base_factories[0], v=0, name="a", keys=(None, "comps")
            ),
            DiracMixtureAccumulatorFactory(factory=self._base_factories[0]),
            DiracMixtureAccumulatorFactory(
                factory=self._base_factories[1], v=0, name="a", keys=("w", "comps")
            ),
            DiracMixtureAccumulatorFactory(
                factory=self._base_factories[1], v=100, name="a", keys=("w", None)
            ),
            DiracMixtureAccumulatorFactory(
                factory=self._base_factories[1], v=0, name="a", keys=(None, "comps")
            ),
            DiracMixtureAccumulatorFactory(factory=self._base_factories[1]),
        ]
        self._accumulators = [
            DiracMixtureAccumulator(
                accumulator=self._base_accs[0], v=0, name="a", keys=("w", "comps")
            ),
            DiracMixtureAccumulator(
                accumulator=self._base_accs[0], v=100, name="a", keys=("w", None)
            ),
            DiracMixtureAccumulator(
                accumulator=self._base_accs[0], v=0, name="a", keys=(None, "comps")
            ),
            DiracMixtureAccumulator(accumulator=self._base_accs[0]),
            DiracMixtureAccumulator(
                accumulator=self._base_accs[1], v=0, name="a", keys=("w", "comps")
            ),
            DiracMixtureAccumulator(
                accumulator=self._base_accs[1], v=100, name="a", keys=("w", None)
            ),
            DiracMixtureAccumulator(
                accumulator=self._base_accs[1], v=0, name="a", keys=(None, "comps")
            ),
    ...
    def test_key_exceptions(self):
        for x in self.type_check_keys:
            ...
            assert (
                str(e.value)
                == "DiracMixtureEstimator requires keys (Tuple[Optional[str], Optional[str]])."
            )
