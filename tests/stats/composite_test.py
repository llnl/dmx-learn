...
class CompositeDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        self._estimators = [
            CategoricalEstimator(name="cat"),
            BinomialEstimator(name="bino"),
            GaussianEstimator(name="g"),
        ]
        self._factories = [
            [
        ...
        self.dist_est = [
            (
                self.eval_dists[0],
                CompositeEstimator(
                    estimators=self._estimators, name="comp", keys="comp_keys"
                ),
            ),
            (
                self.eval_dists[1],
                CompositeEstimator(estimators=self._estimators[:2], name="comp"),
            ),
            (
                self.eval_dists[2],
                CompositeEstimator(estimators=self._estimators[:1], name="comp"),
            ),
        ]
        ...
        self.est_factory = [
            (
                CompositeEstimator(estimators=self._estimators, name="comp"),
                CompositeAccumulatorFactory(factories=self._factories[0], name="comp"),
            ),
            (
                CompositeEstimator(estimators=self._estimators[:2], name="comp"),
                CompositeAccumulatorFactory(
                    factories=self._factories[0][:2], name="comp"
                ),
            ),
            (
                CompositeEstimator(estimators=self._estimators[:1], name="comp"),
                CompositeAccumulatorFactory(
                    factories=self._factories[0][:1], name="comp"
                ),
            ),
        ]
        self.factory_acc = [
            (
                CompositeAccumulatorFactory(
                    factories=self._factories[1], name="comp", keys="comp_key"
                ),
                CompositeAccumulator(
                    accumulators=self._accumulators, name="comp", keys="comp_key"
                ),
            ),
            (
                CompositeAccumulatorFactory(
                    factories=self._factories[1][:2], name="comp", keys="comp_key"
                ),
                CompositeAccumulator(
                    accumulators=self._accumulators[:2], name="comp", keys="comp_key"
                ),
            ),
            (
                CompositeAccumulatorFactory(
                    factories=self._factories[1][:1], name="comp", keys="comp_key"
                ),
                CompositeAccumulator(
                    accumulators=self._accumulators[:1], name="comp", keys="comp_key"
                ),
            ),
        ]
        self.acc_encoder = [
            (
                ...
                CompositeDataEncoder(encoders=self._encoders),
            ),
            (
                CompositeAccumulator(
                    accumulators=self._accumulators[:2], name="comp", keys="comp_key"
                ),
                CompositeDataEncoder(encoders=self._encoders[:2]),
            ),
            (
                CompositeAccumulator(
                    accumulators=self._accumulators[:1], name="comp", keys="comp_key"
                ),
                CompositeDataEncoder(encoders=self._encoders[:1]),
            ),
        ]
    ...
    def test_key_exceptions(self):
        for x in self.type_check_keys:
            ...
            assert (
                str(e.value) == "CompositeEstimator requires keys to be of type 'str'."
            )
