...
from dmx.stats.null_dist import NullAccumulator, NullDataEncoder
...
class MultinomialDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        self.encoder_list = [
            MultinomialDataEncoder(
                encoder=CategoricalDataEncoder(), len_encoder=BinomialDataEncoder()
            ),
            MultinomialDataEncoder(
                encoder=CategoricalDataEncoder(), len_encoder=NullDataEncoder()
            ),
        ]

        self.eval_dists = [
            MultinomialDistribution(
                dist=self.base_dist, len_dist=self.len_dist, len_normalized=True
            ),
            MultinomialDistribution(dist=self.base_dist),
            MultinomialDistribution(
                dist=self.base_dist, len_dist=self.len_dist, name="a"
            ),
            MultinomialDistribution(dist=self.base_dist, name="a"),
        ]
        self.sampler_dist = MultinomialDistribution(
            dist=self.base_dist, len_dist=self.len_dist, len_normalized=True
        )
        self.density_dist_encoder = [
            ...
            (
                MultinomialDistribution(
                    dist=self.base_dist, len_dist=self.len_dist, len_normalized=False
                ),
                self.encoder_list[0],
            ),
        ]
        ...
        self.dist_est = [
            (
                ...
                MultinomialEstimator(
                    ...
                    len_estimator=BinomialEstimator(),
                    name="a",
                ),
            ),
            (
                MultinomialDistribution(
                    dist=self.base_dist, len_dist=self.len_dist, name="a", keys="mkey"
                ),
                MultinomialEstimator(
                    estimator=CategoricalEstimator(),
                    len_estimator=BinomialEstimator(),
        ...
        self.dist_encoder = [
            (x, y) for x, y in zip(self.eval_dists[:2], self.encoder_list)
        ]
        self.est_factory = [
            (
                MultinomialEstimator(
                    estimator=self.base_est,
                    len_estimator=self.len_est,
                    name="m",
                    keys="mkeys",
                ),
                MultinomialAccumulatorFactory(
                    est_factory=self.base_factory,
                    len_factory=self.len_factory,
        ...
        self.factory_acc = [
            (
                ...
                MultinomialAccumulator(
                    accumulator=self.base_acc,
                    len_accumulator=self.len_acc,
                    len_normalized=False,
                    name="m",
                    keys="mkeys",
                ),
            ...
            (
                ...
                MultinomialAccumulator(
                    accumulator=self.base_acc,
                    len_accumulator=NullAccumulator(),
                    len_normalized=False,
                    name="m",
                    keys="mkeys",
                ),
    ...
    def test_key_exceptions(self):
        for x in self.type_check_keys:
            ...
            assert (
                str(e.value)
                == "MultinomialEstimator requires keys to be of type 'str'."
            )
