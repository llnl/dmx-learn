...
from dmx.stats.hmixture import *  # type: ignore[no-redef]  # module exports internal TypeVars
...
class HierarchicalMixtureDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        self._w = [np.ones(5) / 5.0, np.ones(2) / 2.0]
        self._tw = [np.ones((5, 3)) / 3.0, np.ones((2, 4)) / 4.0]
        self._len_dists = [
        ...
        self._encoders = [
            HierarchicalMixtureDataEncoder(
                topic_encoder=self._encs[0], len_encoder=self._len_encs[0]
            ),
            HierarchicalMixtureDataEncoder(
                topic_encoder=self._encs[0], len_encoder=self._len_encs[0]
            ),
            ...
            HierarchicalMixtureDataEncoder(
                topic_encoder=self._encs[1], len_encoder=self._len_encs[1]
            ),
        ]
    ...
    def test_key_exceptions(self) -> None:
        for x in self.type_check_keys:
            with pytest.raises(TypeError) as e:
                HierarchicalMixtureEstimator(
                    [CategoricalEstimator()] * 5, num_mixtures=1, keys=x
                )
