...
class BinomialDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        self.dist_est = [
            (
                BinomialDistribution(
                    p=0.7, n=20, min_val=-3, name="a", keys="bino_key"
                ),
                BinomialEstimator(name="a", keys="bino_key"),
            )
        ]
        self.dist_encoder = [
            (
        ...
        self.sampler_dist = BinomialDistribution(
            p=0.7, n=20, min_val=-3, name="a", keys="bino_key"
        )
        ...
        self.est_factory = [
            (
                BinomialEstimator(min_val=0, max_val=100, name="b", keys="bino"),
                BinomialAccumulatorFactory(
                    min_val=0, max_val=100, name="b", keys="bino"
                ),
            )
        ]
        self.factory_acc = [
            (
                ...
                BinomialAccumulator(min_val=0, max_val=100, name="b", keys="bino"),
            )
        ]
        self.acc_encoder = [
            (
    ...
    def test_key_exceptions(self):
        for x in self.type_check_keys:
            ...
            assert (
                str(e.value) == "BinomialEstimator requires keys to be of type 'str'."
            )
