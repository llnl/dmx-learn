    ...
    def setUp(self) -> None:
        ...
        self._cond_dists = [
            ConditionalDistribution(
                {"a": self._dists[0], "b": self._dists[1], "c": self._dists[2]}
            ),
            ConditionalDistribution({"a": self._dists[0], "b": self._dists[1]}),
        ]
        self._cond_ests = [
            ConditionalDistributionEstimator(
                {x: CategoricalEstimator() for x in ["a", "b", "c"]}
            ),
            ConditionalDistributionEstimator(
                {x: CategoricalEstimator() for x in ["a", "b"]}
            ),
        ]
        self._cond_facts = [
            ConditionalDistributionAccumulatorFactory(
                {x: CategoricalAccumulatorFactory() for x in ["a", "b", "c"]}
            ),
            ConditionalDistributionAccumulatorFactory(
                {x: CategoricalAccumulatorFactory() for x in ["a", "b"]}
            ),
        ]
        self._cond_acc = [
            ConditionalDistributionAccumulator(
                {x: CategoricalAccumulator() for x in ["a", "b", "c"]}
            ),
            ConditionalDistributionAccumulator(
                {x: CategoricalAccumulator() for x in ["a", "b"]}
            ),
        ]
        self._given_dists = [
            MultinomialDistribution(
                CategoricalDistribution({"a": 0.30, "b": 0.30, "c": 0.40}),
                len_dist=CategoricalDistribution({5: 1.0}),
            ),
            MultinomialDistribution(
                CategoricalDistribution({"a": 0.50, "b": 0.50}),
                len_dist=CategoricalDistribution({5: 1.0}),
            ),
        ]
        self._given_ests = [
            MultinomialEstimator(
                CategoricalEstimator(), len_estimator=CategoricalEstimator()
            ),
            MultinomialEstimator(
                CategoricalEstimator(), len_estimator=CategoricalEstimator()
            ),
        ...
        self._given_facts = [
            MultinomialAccumulatorFactory(
                est_factory=CategoricalAccumulatorFactory(),
                len_factory=CategoricalAccumulatorFactory(),
        ...
        self._given_acc = [
            MultinomialAccumulator(
                ...
                len_accumulator=CategoricalAccumulator(),
                len_normalized=False,
            ),
            MultinomialAccumulator(
                accumulator=CategoricalAccumulator(),
        ...
        self._len_dist = [
            CategoricalDistribution({7: 1.0}),
            BinomialDistribution(p=0.30, n=10, min_val=5),
        ]
        self._len_est = [CategoricalEstimator(), BinomialEstimator()]
        self._len_fact = [CategoricalAccumulatorFactory(), BinomialAccumulatorFactory()]
        self._len_acc = [CategoricalAccumulator(), BinomialAccumulator()]

        self.eval_dists = [
            HiddenAssociationDistribution(
                cond_dist=self._cond_dists[0],
                given_dist=self._given_dists[0],
                len_dist=self._len_dist[0],
                name="name",
                keys=("w", "t"),
            ),
            HiddenAssociationDistribution(
            ...
            HiddenAssociationDistribution(
                cond_dist=self._cond_dists[1],
                given_dist=self._given_dists[1],
                len_dist=self._len_dist[1],
                name="name",
                keys=("w", "t"),
            ),
        ]
        ...
        self._factories = [
            HiddenAssociationAccumulatorFactory(
                cond_factory=self._cond_facts[0],
                given_factory=self._given_facts[0],
                len_factory=self._len_fact[0],
                name="name",
                keys=("w", "t"),
            ),
            HiddenAssociationAccumulatorFactory(
            ...
            HiddenAssociationAccumulatorFactory(
                cond_factory=self._cond_facts[1],
                given_factory=self._given_facts[1],
                len_factory=self._len_fact[1],
                name="name",
                keys=("w", "t"),
            ),
        ]
        self._accumulators = [
            HiddenAssociationAccumulator(
                cond_acc=self._cond_acc[0],
                given_acc=self._given_acc[0],
                size_acc=self._len_acc[0],
                name="name",
                keys=("w", "t"),
            ),
            HiddenAssociationAccumulator(
            ...
            HiddenAssociationAccumulator(
                cond_acc=self._cond_acc[1],
                given_acc=self._given_acc[1],
                size_acc=self._len_acc[1],
                name="name",
                keys=("w", "t"),
            ),
        ]
