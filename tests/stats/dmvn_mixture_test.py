    ...
    for seed in seeds:
        ...
        for i in range(sz):
            ...
            if np.all(seq_post[i] == 0):
                ...
                seq_post[i] = np.where(
                    seq_post[i] != 0,
                    np.abs(seq_post[i] - dist.posterior(data[i])) / np.abs(seq_post[i]),
                    dist.posterior(data[i]),
                )
    ...
    for seed in seeds:
        ...
        rv.append(
            np.max(
                np.where(
                    seq_post > 0.0,
                    np.abs(seq_post - fast_post) / np.abs(seq_post),
                    fast_post,
                )
            )
        )
...
class DiagonalGaussianMixtureDistributionTestCase(StatsTestClass):
    ...
    def setUp(self) -> None:
        ...
        mu = [
            np.reshape(np.linspace(-100, 100, num=k * d, endpoint=True), (k, d))
            for k, d in dim_tups
        ]
        ...
        self.eval_dists = [
            DiagonalGaussianMixtureDistribution(
                mu=mu[0], covar=covar[0], w=w[0], name="name", keys=keys[0], tied=True
            ),
            DiagonalGaussianMixtureDistribution(
                mu=mu[1], covar=covar[1], w=w[1], name="name", keys=keys[1]
            ),
            DiagonalGaussianMixtureDistribution(
                mu=mu[2], covar=covar[2], w=w[2], name="name", keys=keys[2]
            ),
            DiagonalGaussianMixtureDistribution(mu=mu[3], covar=covar[3], w=w[3]),
        ]
        self._ests = [
            DiagonalGaussianMixtureEstimator(
                num_components=ncomps[0],
                dim=dims[0],
                tied=True,
                name="name",
                keys=keys[0],
            ...
            DiagonalGaussianMixtureEstimator(
                num_components=ncomps[1], dim=dims[1], name="name", keys=keys[1]
            ),
            DiagonalGaussianMixtureEstimator(
                num_components=ncomps[2], dim=dims[2], name="name", keys=keys[2]
            ),
            DiagonalGaussianMixtureEstimator(num_components=ncomps[3], dim=dims[3]),
        ]
        self._factories = [
            DiagonalGaussianMixtureAccumulatorFactory(
                num_components=ncomps[0],
                dim=dims[0],
                keys=keys[0],
                name="name",
            ...
            DiagonalGaussianMixtureAccumulatorFactory(
                num_components=ncomps[1], dim=dims[1], keys=keys[1], name="name"
            ),
            DiagonalGaussianMixtureAccumulatorFactory(
                num_components=ncomps[2], dim=dims[2], keys=keys[2], name="name"
            ),
            DiagonalGaussianMixtureAccumulatorFactory(
                num_components=ncomps[3], dim=dims[3]
            ),
        ]
        self._accumulators = [
            DiagonalGaussianMixtureAccumulator(
                num_components=ncomps[0],
                dim=dims[0],
                keys=keys[0],
                name="name",
