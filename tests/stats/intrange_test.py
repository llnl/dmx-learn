    ...
    def setUp(self) -> None:
        self.eval_dists = [
            IntegerCategoricalDistribution(
                min_val=0, p_vec=np.ones(3) / 3.0, name="name", keys="keys"
            ),
            IntegerCategoricalDistribution(
                min_val=0, p_vec=np.ones(2) / 2.0, name="name"
            ),
            IntegerCategoricalDistribution(
                min_val=0, p_vec=np.ones(4) / 4.0, keys="keys"
            ),
            IntegerCategoricalDistribution(min_val=0, p_vec=np.ones(5) / 5.0),
        ]
        self._ests = [
            [
            ...
            [
                IntegerCategoricalEstimator(
                    min_val=0, max_val=3, name="name", keys="keys"
                ),
        ...
        self._factories = [
            ...
            [
                IntegerCategoricalAccumulatorFactory(
                    min_val=0, max_val=2, name="name", keys="keys"
                ),
                IntegerCategoricalAccumulatorFactory(),
            ],
        ]
        ...
        self.factory_acc = [
            (f, a) for f, a in zip(self._factories[1], self._accumulators)
        ]
