...
if __name__ == "__main__":
    ...
    topics = [
        GaussianDistribution(mu=x, sigma2=1.0)
        for x in np.linspace(-20 * num_states, 20 * num_states, num_states)
    ]
    ...
    d = HiddenMarkovModelDistribution(
        topics=topics, w=w, transitions=transitions, len_dist=len_dist
    )
    data = d.sampler(1).sample(N)

    # declare estimator
    est = HiddenMarkovEstimator(
        [GaussianEstimator()] * num_states, len_estimator=IntegerCategoricalEstimator()
    )
